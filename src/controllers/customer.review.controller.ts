...
module.exports.toggleVotingLike = async (
  ...
  try {
    ...
    if (!reviewID) throw new Error400("Required review id from body !");

    // find one review by review id;
    const review = await Review.findOne({ _id: ObjectId(reviewID) });

    if (!review) throw new Error400("Review not found !");
    ...
    if (!result) throw new Error400("Operation failed !");
...
module.exports.getProductDetails = async (
  ...
  try {
    ...
    const getOrder = await ORDER_TABLE.findOne({
      $and: [
        { _id: ObjectId(oid) },
    ...
    if (!getOrder) return next(new Error400("ORDER_TABLE not found !"));
