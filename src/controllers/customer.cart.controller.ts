...
async function addToCartSystem(
  ...
  try {
    const { _id: customerId, email }: any = req.decoded;
    const body = req.body;
    ...
    if (action !== "toCart") throw new Error400("Required cart action !");

    if (!productId || !sku)
      throw new Error400(
        "Required product id, sku in body !"
      );

    const { stock, supplierId } = await isProduct(productId, sku);
...
async function getCartContextSystem(
  ...
  try {
    ...
    let customer = await Customer.findOne({ _id: ObjectId(customerId) });

    const cartData = NodeCache.getCache(`${email}_cartProducts`);
    const uri:any = req?.get("Origin");

    if (cartData) {
      cart = cartData;
    } else {
      cart = await ShoppingCart.aggregate(shopping_cart_pipe(customer?._id, uri));
...
async function updateCartProductQuantitySystem(
  ...
  try {
    ...
    const { productId, sku, cartId, quantity } = req?.body;
    ...
    const { stockQuantity } = await isProduct(productId, sku);
    ...
    if (!result) throw new Error("Failed to update quantity !");
...
async function deleteCartItemSystem(
  ...
  try {
    ...
    if (!cartId) throw new Error400("Required cart id!");

    if (!ObjectId.isValid(cartId))
      throw new Error400("Cart id is not valid !");

    let deleted = await ShoppingCart.deleteOne({
      $and: [{ _id: ObjectId(cartId) }, { customerId: ObjectId(customerId) }],
    });

    if (!deleted) throw new Error(`Internal server error!`);

    const cartInCached = NodeCache.getCache(`${email}_cartProducts`);

    if (cartInCached) {
      const filteredItems = cartInCached.filter(
        (item: any) => item?._id !== cartId
      );
